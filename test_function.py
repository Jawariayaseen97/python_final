...
class TestJson(TestCase):
    ...
    def assertJson(self, function, expected, *args):
        result = function(*args)
        self.assertEqual(result, expected, f"Expected {expected}, got {result}")

    def test_json_calories_combo_complex_id(self):
        self.assertJson(comp_cal_counter_json, 1770, 'combo-1', 'combo-2')

    def test_json_calories_combo_complex_name(self):
        self.assertJson(comp_cal_counter_json, 1770, 'cheesy combo', 'veggie combo')

    def test_json_calories_complex_mix_id(self):
        self.assertJson(comp_cal_counter_json, 1300, 'meal-1', 'combo-2
